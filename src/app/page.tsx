...
export default function Home() {
  ...
  const handlePagination = (type: string | number) => {
    if (type === 'pre') {
      console.log(currentPage, 'page-62')
      if (currentPage > 1) {
  ...
  const hurryUp = async () => {
    ...
    console.log(resJson.count, 'page-120')
  }
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 py-8">
      ...
      <div className="flex justify-between text-2xl font-bold text-zinc-800">Blog News
        <Dialog>
          <DialogTrigger asChild><Button className="cursor-pointer" variant="outline">提交RSS</Button></DialogTrigger>
          <DialogContent>
            <DialogHeader>
      ...
      <main className="flex flex-col gap-2 row-start-2 items-center sm:items-start">
        {posts?.results?.map(item => {
          ...
          return <div className="w-3xl cursor-pointer hover:bg-blue-50 p-2 rounded-lg" key={title} onClick={() => handleClick(link)}>
            <div>
