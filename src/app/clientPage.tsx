...
export default function Home() {
  const form = useForm()
  const [posts, setPosts] = useState({ total: 0, page: 0, config: { updateAt: '' }, results: [] })
  const [config, setConfig] = useState({ updateAt: '' })
    ...
    <div className="max-w-3xl mx-auto p-4 py-8">
      ...
      <div className="flex justify-between items-center text-2xl font-bold text-zinc-800 sticky  top-0  bg-white pt-4 pb-4 z-10">
        <div className="md:flex items-center">
          ...
          <span className="text-xs font-normal flex-1 absolute opacity-60 mr-1 md:relative md:ml-3">最近更新：{config?.updateAt || '-'} | 当前收录：{totalRss || '-'}个</span>
        </div>
        <Dialog>
